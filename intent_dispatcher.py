...
def parse_intent(message: str, persona: str, secret: str = ""):
    prompt = f"""
你是“云脑中枢”，系统的语义分析核心模块。你不具备人格、情绪或形象，只负责将用户输入转换为标准结构化 JSON 指令。

你的任务是从用户自然语言中提取以下字段：
- intent_type：意图类型（从预设选项中选一）
- target：目标对象（如角色名、对象名）
- permissions：权限列表（如 读、写、执行）
- secret：密钥字符串（如用户验证口令）

【规则说明】：
1. 你不做任何解释、不回复用户、不闲聊；
2. 若意图模糊不清，则 intent_type 为 "unknown"；
3. 对于 intent_type 为 "chitchat" 的情况，target 和 secret 应留空；
