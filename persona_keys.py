...
# ✅ 检查 persona 是否存在 + 是否 active
def is_persona_active(persona: str) -> bool:
    try:
        result = supabase.table(TABLE).select("active").eq("persona", persona).limit(1).execute()
        if not result.data:
            return False
        return result.data[0]["active"] == True
    except:
        return False
